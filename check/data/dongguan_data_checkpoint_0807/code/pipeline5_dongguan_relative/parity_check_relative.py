#!/usr/bin/env python3
"""Verify Dongguan relative-action checkpoint processor + optional policy load."""
...
def check_processor_contract(checkpoint_dir: Path) -> dict[str, Any]:
    ...
    if not stats_ok:
        ...
        notes.append(
            "Infer: use dongguan_inference bootstrap (see infer_dongguan.txt); "
            "do not load with pipeline3 absolute or pipeline4 [-5,0] modality."
        )

    ok = not errors and all(c.get("ok", True) for c in checks)
    return {
        "checkpoint": str(checkpoint_dir.resolve()),
        "pipeline": "pipeline5_dongguan_relative",
        "ok": ok,
        "checks": checks,
        "errors": errors,
        "notes": notes,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pipeline5 Dongguan relative parity check.")
    ...
    print(f"Dongguan relative parity: ok={report['ok']}")
