"""Verify checkpoint processor contract matches Quanta X1 train settings."""
...
def check_processor_contract(checkpoint_dir: Path) -> dict[str, Any]:
    ...
    if EMBODIMENT_TAG_VALUE not in modality_configs:
        ...
        checks.append(_check_keys("video_keys", emb["video"]["modality_keys"], EXPECTED_NEW_EMBODIMENT["video_keys"]))
        checks.append(_check_keys("state_keys", emb["state"]["modality_keys"], EXPECTED_NEW_EMBODIMENT["state_keys"]))
        checks.append(
        ...
        action_reps = [cfg["rep"] for cfg in emb["action"].get("action_configs", [])]
...
def check_loaded_policy(checkpoint_dir: Path) -> dict[str, Any]:
    ...
    for modality in ("video", "state", "action", "language"):
        ...
        policy_checks.append({"name": f"policy_{modality}_keys", "ok": ok, "expected": exp_keys, "actual": act_keys})
        if not ok:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Quanta X1 train/inference parity check.")
    parser.add_argument(
        "--checkpoint",
        type=Path,
