"""4-bit QLoRA hooks for pipeline7: LLM + ViT LoRA (r=32) on frozen NF4 backbone."""
...
_QLORA_PATCH_VERSION = "pipeline7_qlora_v1"
_APPLIED = False
...
def _build_4bit_vlm(model, transformers_loading_kwargs: dict[str, Any] | None):
    ...
    print(
        "[pipeline7_qlora] loading VLM in 4-bit NF4: "
        f"{model_name!r}",
        flush=True,
    )
...
def install_qlora_hooks(
    *,
    ...
    patched_create_model._pipeline7_qlora_patch_version = _QLORA_PATCH_VERSION
...
def apply_qlora_to_gr00t_model(
    ...
    *,
    ...
    print(
        "[pipeline7_qlora] applied PEFT LoRA on LLM+ViT; "
...
def load_qlora_finetuned_model(
    ...
    *,
    ...
    print(
        f"[pipeline7_qlora] rebuilding QLoRA model from base={base_model_path!r} "
