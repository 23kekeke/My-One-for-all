"""Load finetuned Dongguan pipeline7 eef-only QLoRA GR00T policy."""
...
from dongguan_eef_inference.constants import BASE_MODEL, EMBODIMENT_TAG_VALUE, NUM_INFERENCE_TIMESTEPS
from dongguan_eef_inference.env import ensure_dongguan_infer_imports


def _checkpoint_looks_complete(checkpoint_path: Path) -> bool:
    """Reject partial Drive downloads (*.drivedownload / missing shards)."""
    index = checkpoint_path / "model.safetensors.index.json"
    if not index.is_file():
        return (checkpoint_path / "model.safetensors").is_file() or (
    ...
    for shard in shards:
        path = checkpoint_path / shard
        if not path.is_file():
            return False
        if path.name.endswith(".drivedownload") or path.suffix == ".drivedownload":
            return False
        # sibling incomplete download marker
        if (checkpoint_path / f"{shard}.drivedownload").exists():
            return False
    return True
...
def build_gr00t_policy(
    *,
    ...
    if not _checkpoint_looks_complete(checkpoint_path):
        raise FileNotFoundError(
            f"Checkpoint incomplete (missing/partial shards): {checkpoint_path}. "
            "Wait for model-*.safetensors download to finish, or use checkpoint-13000."
        )
...
def load_policy(
    ...
    *,
    ...
    from dongguan_eef_inference.constants import ACTION_KEYS

    if action_keys != list(ACTION_KEYS):
        raise RuntimeError(
            f"Unexpected action keys {action_keys}; expected eef-only {list(ACTION_KEYS)}"
        )
    return policy
