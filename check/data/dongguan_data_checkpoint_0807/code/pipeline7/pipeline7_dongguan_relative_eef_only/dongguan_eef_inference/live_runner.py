"""Dongguan pipeline7 eef-only shadow/live runner (wraps quanta_biman_inference.live_runner).

Action is 20D (eef+gripper only) → execute_via must be end_pose (no joint targets).
Adds per-task home preposition before live execute cycles.
"""
...
_PIPELINE7 = Path(__file__).resolve().parents[1]
if str(_PIPELINE7) not in sys.path:
    sys.path.insert(0, str(_PIPELINE7))

from dongguan_eef_inference import bootstrap_infer
...
def _inject_dongguan_home_args(parser: argparse.ArgumentParser) -> None:
    ...
    for action in parser._actions:
        ...
        if action.dest == "execute_via":
            ...
            action.help = (
                "Policy motion path (pipeline7 eef-only: must be end_pose; "
                "absolute eef_9d via set_end_pose / execute_end_pose_trajectory)"
            )
    ...
    for action in parser._actions:
        ...
        if action.dest == "preposition_max_joint_delta_rad":
            action.help = "(unused for Dongguan end_pose home) kept for pipeline3 compat"
...
def validate_dongguan_home_args(args: argparse.Namespace) -> None:
    ...
    if execute_via != "end_pose":
        raise ValueError(
            "pipeline7 eef-only has no joint actions; use --execute-via end_pose "
            f"(got {execute_via!r})"
        )
...
def run_dongguan_live_runner(args: argparse.Namespace) -> dict[str, Any]:
    from dongguan_eef_inference.preposition import run_dongguan_home_preposition
    ...
    print("dongguan pipeline7 eef-only live_runner starting...", flush=True)
    ...
    from dongguan_eef_inference.end_pose_bias import set_task0_pitch_context
    ...
    if task_index == 0 and abs(pitch_deg) > 1e-9:
        print(
            f"[pipeline7] task0 right end_pose pitch-up={pitch_deg:.2f}deg "
            "(applied before SDK home + live trajectory; task1/2 unchanged)",
            flush=True,
        )
    elif task_index == 0:
        print(
            "[pipeline7] task0: no end_pose pitch compensation "
            "(pass --task0-end-pose-pitch-up-deg DEG to enable)",
            flush=True,
        )
    elif task_index in (1, 2):
        print(
            f"[pipeline7] task_index={task_index}: no end_pose pitch bias",
            flush=True,
        )
    ...
    manifest["pipeline"] = "pipeline7_dongguan_relative_eef_only"
    manifest["action_dim"] = 20
    ...
    print(f"execute_via={execute_via} (eef-only)", flush=True)
    ...
    try:
        ...
        if do_home:
            ...
            # Let camera/gRPC streams recover after long dual home (~15s of set_end_pose).
            settle_after_home = 1.0
            print(
                f"[dongguan home] settling {settle_after_home:.1f}s before first capture...",
                flush=True,
            )
            time.sleep(settle_after_home)
            ...
            cycle_reports.append(
                {
                    ...
                    "right_gripper": step0["right"]["gripper_position"],
                    "left_gripper": step0["left"]["gripper_position"],
                }
            )
            print(
                ...
                f"grip_r={step0['right']['gripper_position']:.3f}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("\nDongguan pipeline7 eef-only live runner complete.")
