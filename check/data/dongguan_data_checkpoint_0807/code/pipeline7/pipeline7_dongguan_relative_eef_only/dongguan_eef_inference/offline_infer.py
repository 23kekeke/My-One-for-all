"""Offline inference on Dongguan LeRobot episodes (pipeline7 eef-only; no robot)."""
...
from dongguan_eef_inference.constants import DEFAULT_CHECKPOINT, INFERENCE_TMP, MULTI_DATASET
...
def get_modality_configs():
    ...
    return dongguan_eef_only_relative_config.dongguan_eef_only_relative_config
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dongguan pipeline7 eef-only offline infer smoke.")
    ...
    print(
        f"  episode={report['episode_index']} step={report['step_index']} "
        f"task_index={report['task_index']} action_keys={report['action_keys']}"
    )
