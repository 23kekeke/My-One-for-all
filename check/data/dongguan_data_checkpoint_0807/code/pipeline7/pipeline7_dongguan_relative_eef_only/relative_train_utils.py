"""Shared helpers for pipeline7_dongguan_relative_eef_only GR00T training."""
...
PIPELINE5 = ROOT / "pipeline5_dongguan_relative"
...
def setup_gr00t_env() -> dict[str, str]:
    env = os.environ.copy()
    pipeline_paths = [str(GR00T_REPO), str(PIPELINE2), str(PIPELINE5), str(PIPELINE7)]
...
def ensure_gr00t_imports() -> None:
    ...
    for path in (str(PIPELINE7), str(PIPELINE5), str(PIPELINE2), str(GR00T_REPO)):
...
def load_relative_modality_config() -> None:
    ...
    if "dongguan_eef_only_relative_config" in sys.modules:
        importlib.reload(sys.modules["dongguan_eef_only_relative_config"])
    else:
