...
def update_policy(
    ...
    # Optional input-only noise for robust behavior cloning. This is applied
    # after normalization, so the value is measured in normalized state units.
    # Action targets are deliberately left untouched.
    state_noise_std = float(os.environ.get("ACT_STATE_NOISE_STD", "0"))
    if state_noise_std < 0:
        raise ValueError("ACT_STATE_NOISE_STD must be non-negative")
    if is_main_process:
        logging.info(f"Normalized observation.state noise std: {state_noise_std}")
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    for _ in range(step, cfg.steps):
        ...
        if state_noise_std > 0 and "observation.state" in batch:
            state = batch["observation.state"]
            batch["observation.state"] = state + torch.randn_like(state) * state_noise_std
