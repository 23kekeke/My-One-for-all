...
import functools
from concurrent.futures import ThreadPoolExecutor, as_completed, ProcessPoolExecutor, ProcessPoolExecutor

# ---- Optional faster dependencies ----
...
try:
    from decord import VideoReader as _DecordVR, cpu as _decord_cpu
    _DECORD_CTX = _decord_cpu(0)
    _HAS_DECORD = True
except ImportError:
    _DecordVR = None
    _DECORD_CTX = None
...
def _json_load(path: Path):
    """Load JSON file, using orjson if available for speed.

    Tries orjson first; falls back to json.load on failure.
    """
    if _HAS_ORJSON:
        try:
...
def _read_image_cv2(img_path: Path):
    """Read a single image, returns (C,H,W) uint8 array.

    Tries cv2 first for speed; falls back to PIL for format compatibility.
    """
    img_bgr = cv2.imread(str(img_path), cv2.IMREAD_COLOR)
    if img_bgr is not None:
        return cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB).transpose(2, 0, 1)

    # Fallback to PIL for formats cv2 cannot handle (e.g. TIFF, BMP variants)
...
def _read_images_from_frame(frame: Dict[str, Any], episode_dir: Path,
                             camera_name_mapping: dict = None):
    """Read all camera images for one frame. Returns {cam_name: (C,H,W) array}."""
    result = {}
    cam_entries = frame.get('images', {})
    if not cam_entries:
        return result

    # Single camera: no thread pool overhead
    if len(cam_entries) == 1:
        for cam_name, img_filename in cam_entries.items():
            img_path = episode_dir / img_filename
            arr = _read_image_cv2(img_path)
            if arr is not None:
                mapped = camera_name_mapping.get(cam_name, cam_name) if camera_name_mapping else cam_name
                result[mapped] = arr
        return result

    # Multiple cameras: use thread pool for parallel I/O
    tasks = []
    for cam_name, img_filename in cam_entries.items():
        img_path = episode_dir / img_filename
        mapped = camera_name_mapping.get(cam_name, cam_name) if camera_name_mapping else cam_name
        tasks.append((img_path, mapped))

    with ThreadPoolExecutor(max_workers=min(len(tasks), 8)) as pool:
        futures = {pool.submit(_read_image_cv2, p): n for p, n in tasks}
        for future in as_completed(futures):
            arr = future.result()
            if arr is not None:
                result[futures[future]] = arr
    return result
...
ROBOT_DATA_CONFIG = {
    ...
    "quanta_x1_raw_joints": {
        # Joint-only mode: raw joint positions + grippers, no end-effector pose
        "action_order": [
            "left_arm_joint_positions",   # 6 dims
            "left_gripper_position",       # 1 dim
            "right_arm_joint_positions",   # 6 dims
            "right_gripper_position",      # 1 dim
        ],
        "state_order": [
...
def get_robot_config(robot_type: str) -> Dict[str, Any]:
    ...
    """
    # Try to match robot_type (supports partial matching, prefer longer keys)
    robot_type_lower = robot_type.lower()
    for key in sorted(ROBOT_DATA_CONFIG.keys(), key=lambda k: -len(k)):
...
def extract_action_v3(frame: Dict[str, Any], robot_type: str = DEFAULT_ROBOT_TYPE) -> Optional[np.ndarray]:
    ...
    # Resolve source name: try master_ prefix first, fall back to non-master
    def _resolve_source(src_name, data_dict):
        if src_name in data_dict:
            return src_name
        if src_name.startswith("master_"):
            alt = src_name[len("master_"):]
            if alt in data_dict:
                return alt
        return src_name
    ...
    for action_field in action_order:
        ...
        source_name = _resolve_source(source_config["source"], action_dict)
...
def extract_state_v3(frame: Dict[str, Any], joint_indices: Optional[List[int]] = None, robot_type: str = DEFAULT_ROBOT_TYPE) -> Optional[np.ndarray]:
    ...
    # Resolve source name: try master_ prefix first, fall back to non-master
    def _resolve_state_source(src_name, data_dict):
        if src_name in data_dict:
            return src_name
        if src_name.startswith("master_"):
            alt = src_name[len("master_"):]
    ...
    for state_field in state_order:
        ...
        source_name = _resolve_state_source(source_config["source"], obs)
        source_type = source_config["type"]
        dim = source_config.get("dim", 0)
...
Important notes:
  ...
  - Sensor data: New sensor data will be saved as additional observation fields
    ...
    parser.add_argument("--processes", "-j", type=int, default=1,
                        help="Number of parallel worker processes for episode processing (default: 1)")
...
def load_dataset_metadata(input_dir: Path) -> Dict[str, Any]:
    ...
    return _json_load(metadata_file)
...
def load_episode(input_dir: Path, episode_id: int) -> Dict[str, Any]:
    ...
    episode_data = _json_load(episode_json)
    ...
    if storage_format == 'video' and 'video_files' in episode_data:
        # Open all video files (decord preferred, cv2 fallback)
        video_readers = {}
        for cam_name, video_file in episode_data['video_files'].items():
            video_path = episode_dir / video_file
            if not video_path.exists():
                continue
            vid_path_str = str(video_path)
            if _HAS_DECORD:
                try:
                    vr = _DecordVR(vid_path_str, ctx=_DECORD_CTX)
                    video_readers[cam_name] = ('decord', vr)
                    print(f"  Load video: {cam_name}.mp4 ({len(vr)} frames) [decord]")
                    continue
                except Exception as e:
                    print(f"  ⚠️  decord failed for {cam_name}, falling back to cv2: {e}")
            cap = cv2.VideoCapture(vid_path_str)
            if cap.isOpened():
                video_readers[cam_name] = ('cv2', cap)
                frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                print(f"  Load video: {cam_name}.mp4 ({frame_count} frames) [cv2]")
            else:
                print(f"  ⚠️  Warning: Unable to open video file: {video_path}")
        episode_data['_video_readers'] = video_readers
...
def process_single_frame(
    ...
    Args:
        ...
        is_v3_format: Pre-detected V3 format flag (avoids per-frame re-detection)
    """
    result = {}
    
    # Detect if it is V3 format (latest format)
    is_v3 = is_v3_format if is_v3_format is not None else _is_v3_format(frame)
    ...
    if storage_format == 'video' and video_captures:
        # Read frames from video (decord preferred, cv2 fallback)
        for cam_name, frame_index in frame.get('images', {}).items():
            if cam_name not in video_captures:
                ...
                continue
            reader_type, reader = video_captures[cam_name]
            mapped_cam_name = camera_name_mapping.get(cam_name, cam_name) if camera_name_mapping else cam_name
            
            if reader_type == 'decord':
                try:
                    arr = reader[frame_index].asnumpy()  # (H,W,C) RGB
                    result['images'][mapped_cam_name] = arr.transpose(2, 0, 1)
                except Exception:
                    print(f"  ⚠️  Warning: decord failed to read frame {frame_index} ({cam_name})")
            else:
                reader.set(cv2.CAP_PROP_POS_FRAMES, frame_index)
                ret, img_bgr = reader.read()
                if ret:
                    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
                    result['images'][mapped_cam_name] = img_rgb.transpose(2, 0, 1)
                else:
                    print(f"  ⚠️  Warning: Unable to read frame {frame_index} ({cam_name}) from video")
    else:
        # Read frames from image files (cv2 with threaded parallel I/O)
        result['images'] = _read_images_from_frame(frame, episode_dir, camera_name_mapping)
...
def extract_all_states(
    ...
    """
    _is_v3 = is_v3_format if is_v3_format is not None else _is_v3_format(frames[0]) if frames else False
    states = []
    for frame in frames:
        state = None
...
def generate_future_actions(
    ...
    all_states = extract_all_states(frames, joint_indices, is_new_format, robot_type=robot_type, is_v3_format=is_v3_format)
...
def create_lerobot_dataset(
    ...
    if action_dim and action_dim > 0:
        ...
        order_names_map = config.get("order_names", {})
        for action_name in action_order:
            if action_name in order_names_map:
                action_names.extend(order_names_map[action_name])
            elif action_name == "master_left_ee_cartesian_pos":
    ...
    # Add camera features (per-camera resolution)
    for cam in camera_names:
        h, w = camera_shapes.get(cam, (480, 640))
...
def _process_episode_worker(args):
    """Worker for ProcessPoolExecutor: processes one episode independently.

    Loads episode JSON, extracts states/actions, reads all images,
    saves image tensors to temp .npy files, returns metadata.
    """
    (input_dir_str, output_dir_str, ep_idx, robot_type, joint_indices,
     future_action_chunk_size, relative_start, delta_action) = args
    ...
    video_readers = episode_data.get('_video_readers', {})
    ...
    is_v3 = False
    if frames:
        is_v3 = _is_v3_format(frames[0])

    # Detect action
    has_action = _has_action_field(episode_data)

    # Generate future actions if needed
    future_actions = None
    if not has_action:
        future_actions = generate_future_actions(
            frames, joint_indices, is_new_format,
            future_action_chunk_size, robot_type=robot_type,
            is_v3_format=is_v3
        )

    processed_states = []
    processed_actions = []
    image_arrays = {}  # cam_name -> list of (C,H,W) uint8 arrays
    frames_kept = 0
    ...
    for i in range(total_frames):
        ...
        processed = process_single_frame(
            frame_data, episode_dir, joint_indices,
            is_new_format,
            storage_format=storage_format,
            video_captures=video_readers,
            override_action=override_action,
            robot_type=robot_type,
            is_v3_format=is_v3
        )

        processed_states.append(processed.get('state'))
        processed_actions.append(processed.get('action'))

        for cam_name, img_array in processed.get('images', {}).items():
            if cam_name not in image_arrays:
                image_arrays[cam_name] = []
            image_arrays[cam_name].append(img_array)
    ...
    if storage_format == 'video' and video_readers:
        for _cam_name, (reader_type, reader) in video_readers.items():
            if reader_type == 'cv2':
                reader.release()

    # Save images to temp .npy files
    temp_dir = Path(output_dir_str) / f"_temp_ep_{ep_idx}"
    temp_dir.mkdir(parents=True, exist_ok=True)
    image_paths = {}
    image_counts = {}
    for cam_name, arrays in image_arrays.items():
        npy_path = temp_dir / f"{cam_name}.npy"
        all_arr = np.array(arrays, dtype=np.uint8)  # (N, C, H, W)
        np.save(str(npy_path), all_arr)
        image_paths[cam_name] = str(npy_path)
        image_counts[cam_name] = len(arrays)
    ...
    return {
        ...
        'num_total': total_frames,
        'task_str': episode_data.get('task', 'unknown'),
        'storage_format': storage_format,
        'has_future_actions': future_actions is not None,
    }
...
def convert_dataset(args):
    ...
    # Check robot_type consistency with episode model. The specialized
    # ``quanta_x1_raw_joints`` conversion uses the same physical robot as
    # ``quanta_x1``; only the exported state/action representation differs.
    expected_episode_model = args.robot_type.removesuffix("_raw_joints")
    for ep_idx in selected_episodes:
        if ep_idx >= len(metadata['episodes']):
            continue
        ep_model = metadata['episodes'][ep_idx].get('model', '')
        if ep_model.lower() != expected_episode_model.lower():
            ...
            if test_episode.get('_has_frames') and '_first_frame' in test_episode:
                ...
                if _is_v3_format(first_frame):
                    ...
                    if test_state is not None:
                        ...
                        for state_field in state_order:
                            if state_field in order_names_map:
                                state_names_list.extend(order_names_map[state_field])
                            elif state_field == "master_left_ee_cartesian_pos":
    ...
    # Detect actual camera resolutions from first episode
    camera_shapes: dict[str, tuple] = {}
    for ep_idx in selected_episodes[:1]:
        test_ep = load_episode_metadata(input_dir, ep_idx)
        ep_dir = test_ep['_episode_dir']
        storage_format = 'images'
        # check if video storage
        ep_json = ep_dir / "episode.json"
        if ep_json.exists():
            raw = _json_load(ep_json)
            storage_format = raw.get('storage_format', 'images')
        if storage_format == 'video':
            # video storage: get resolution from video files
            video_files = {}
            if ep_json.exists() and 'video_files' in raw:
                video_files = raw['video_files']
            for cam_name in actual_camera_names:
                ...
                if video_file:
                    ...
                    if _HAS_DECORD:
                        try:
                            vr = _DecordVR(vid_str, ctx=_DECORD_CTX)
                            arr = vr[0].asnumpy()
                            h, w = arr.shape[:2]
                        except Exception:
                            pass
                    if w == 0:
                        ...
                        if cap.isOpened():
                            ...
                            cap.release()
                    if w > 0:
                        camera_shapes[cam_name] = (h, w)
                        print(f"  Camera {cam_name}: {w}x{h}")
                if cam_name not in camera_shapes:
                    camera_shapes[cam_name] = (480, 640)
        elif test_ep.get('_has_frames') and '_first_frame' in test_ep:
            # image storage: get resolution from the first image file
            first_frame = test_ep['_first_frame']
            for cam_name in actual_camera_names:
                img_info = first_frame.get('images', {}).get(cam_name)
                if isinstance(img_info, str) and img_info:
                    full_path = ep_dir / img_info
                    if full_path.exists():
                        arr = _read_image_cv2(full_path)
                        if arr is not None:
                            _, h, w = arr.shape
                            camera_shapes[cam_name] = (h, w)
                            print(f"  Camera {cam_name}: {w}x{h}")
                if cam_name not in camera_shapes:
                    camera_shapes[cam_name] = (480, 640)
        break
    for cam in actual_camera_names:
        if cam not in camera_shapes:
            camera_shapes[cam] = (480, 640)
            print(f"  Camera {cam}: using default 640x480")
    ...
    if args.processes > 1 and len(selected_episodes) > 1:
        import shutil
        print(f"  Using {args.processes} parallel worker processes...")
        ...
        for ep_idx in selected_episodes:
            if ep_idx >= len(metadata['episodes']):
                ...
                continue
            worker_args.append((
                str(input_dir), str(output_dir), ep_idx, args.robot_type,
                joint_indices, args.future_action_chunk_size,
                args.relative_start, args.delta_action
            ))

        with ProcessPoolExecutor(max_workers=args.processes) as pool:
            results = list(pool.map(_process_episode_worker, worker_args))
        results.sort(key=lambda r: r['ep_idx'])

        for result in results:
            ep_idx = result['ep_idx']
            task_str = result['task_str']
            total_frames = result['num_total']

            print(f"\nProcessing Episode {ep_idx}: {task_str}")
            print(f"  Frame number: {total_frames}")

            # Load image data from temp .npy files
            image_data = {}
            for cam_name, npy_path in result['image_paths'].items():
                image_data[cam_name] = np.load(npy_path)  # (N, C, H, W)
            ...
            for frame_i in range(num_frames):
                ...
                # State
                state_i = result['states'][frame_i]
                if "observation.state" in dataset.features:
                    if state_i is not None:
                        if first_state is None:
                            first_state = state_i.copy()
                        if args.relative_start and first_state is not None:
                            state_i = state_i - first_state
                        frame["observation.state"] = torch.from_numpy(state_i).type(torch.float32)
                    else:
                        s_shape = dataset.features["observation.state"]["shape"]
                        if isinstance(s_shape, tuple) and len(s_shape) == 1:
                            frame["observation.state"] = torch.zeros(s_shape[0], dtype=torch.float32)

                # Action
                action_i = result['actions'][frame_i]
                if action_i is not None:
                    if first_action is None:
                        first_action = action_i.copy()
                    if args.relative_start and first_action is not None:
                        action_i = action_i - first_action
                    if args.delta_action and state_i is not None and not result['has_future_actions']:
                        action_i = action_i - state_i
                    # Verify/adjust dimension
                    if action_dim is not None:
                        actual = len(action_i)
                        if actual != action_dim:
                            if actual < action_dim:
                                padding = np.zeros(action_dim - actual, dtype=np.float32)
                                action_i = np.concatenate([action_i, padding])
                            else:
                                action_i = action_i[:action_dim]
                    frame["action"] = torch.from_numpy(action_i).type(torch.float32)
                else:
                    a_shape = dataset.features["action"]["shape"]
                    if isinstance(a_shape, tuple) and len(a_shape) == 1:
                        frame["action"] = torch.zeros(a_shape[0], dtype=torch.float32)

                # Images
                for cam_name, imgs in image_data.items():
                    frame[f"observation.images.{cam_name}"] = imgs[frame_i]

                dataset.add_frame(frame)
                frames_added_count += 1

                if frames_added_count % 100 == 0:
                    print(f"    Processed {frames_added_count}/{num_frames} frames...")

            dataset.save_episode()
            print(f"  ✓ Episode {ep_idx} saved to LeRobot dataset ({frames_added_count}/{total_frames} frames)")

            # Cleanup temp dir
            for cam_name, npy_path in result['image_paths'].items():
                temp_dir_path = Path(npy_path).parent
    ...
    for ep_idx in selected_episodes:
        ...
        storage_format = episode_data.get('_storage_format', 'images')
        video_captures = episode_data.get('_video_readers', {})
        ...
        # Release video resources
        if storage_format == 'video' and video_captures:
            for _cam_name, (reader_type, reader) in video_captures.items():
                if reader_type == 'cv2':
                    reader.release()
