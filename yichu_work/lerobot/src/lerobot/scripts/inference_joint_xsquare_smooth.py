...
def generate_smooth_curve(start_val, end_val, start_slope, end_slope, transition_steps, smooth_factor=1.0):
    """
    Cubic polynomial interpolation for smooth transitions between action batches.

    :param start_val: Start point value (list/np.array)
    :param end_val: End point value (list/np.array)
    :param start_slope: Slope before start point (np.array)
    :param end_slope: Slope after end point (np.array)
    :param transition_steps: Number of transition steps
    :param smooth_factor: Smoothness factor (0~1, smaller = smoother)
    :return: Smoothed transition curve (np.array), shape=(transition_steps+1, dim)
    """
...
def inference(cfg: InferenceConfig) -> None:
    """Main inference function with action chunk smoothing for smooth robot control."""
    ...
    try:
        ...
        while time.time() - start_time < inference_time_s:
            ...
            # Smoothing: batch-internal convolution smoothing
            ...
            # Smoothing: batch-to-batch cubic interpolation transition
            if (
            ...
            # Execute each smoothed action step
            for action_step in smoothed_batch:
                ...
                # Track for next batch smoothing
