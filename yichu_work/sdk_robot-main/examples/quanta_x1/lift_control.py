...
def move_lift_to_target(robot: Robot, target: float):
    """将升降机构移动到目标绝对位置。
...
def main(
    ...
    action: Annotated[str, typer.Option(help="操作类型: move(运动), stream(数据流), moveto(移动到绝对位置)")] = "move",
    direction: Annotated[str, typer.Option(help="移动方向: up(上升), down(下降)")] = "down",
    distance: Annotated[float, typer.Option(help="移动距离（米）")] = 0.05,
    target: Annotated[float, typer.Option(help="目标绝对位置（米），用于 moveto 模式")] = 0.31,
):
    """量子1号Pro 升降机构控制命令行工具。
    ...
    用法示例:
      ...
      python lift_control.py --action moveto --target 0.31 --server 192.168.36.116:50051
    ...
    if action == "move":
        ...
        move_lift_to_target(robot, target)
    elif action == "stream":
        stream_lift_joint_states(robot)
    else:
        print(f"未知操作: {action}")
        print("有效操作: move, moveto, stream")
        return
