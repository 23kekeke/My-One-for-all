...
执行命令是### 仅joint录制
```sh
python /home/yichu/yichu_work/data_collection/record_joint_.py \
  --server 192.168.36.246:50051 \
  --out /home/yichu/yichu_work/datasets/joint_record \
  --task "pull the door"
```
            ...
            for msg in stream:
                ...
                if self._joint_indices is None:
                    ...
                    if not left_idx and not right_idx:
                        ...
                        self.is_recording = False
                        break
                    self._joint_indices = {"left": left_idx, "right": right_idx}
